...
const UploadContent: FC<UploadContentProps> = ({ visibility, onClose }) => {
    ...
    const addSvg = async (url: string) => {
        ...
        const width = viewBox.length === 4 ? +viewBox[2] : +(ele.getAttribute('width') || 100);
        const height = viewBox.length === 4 ? +viewBox[3] : +(ele.getAttribute('height') || 100);
                ...
                >
                    {images.map((item, idx) => (
                        <div
                            ...
                            onClick={() => (item.type === 'image' ? addImage(item.url) : addSvg(item.url))}
                        >
